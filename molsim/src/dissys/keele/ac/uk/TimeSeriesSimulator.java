...
public class TimeSeriesSimulator {
	...
	{
		CTimeSeries timeSeries=TimeSeriesSimulator.timeCourseSimulation("abcomm.xml", 1000, 80000);
		
		String[][] csvMatrix=TimeSeriesSimulator.createReport(timeSeries);
		TimeSeriesSimulator.writeCsv(new File("abcomm_all.csv"), csvMatrix);
		String[] columns="A,B,IPTG,ATC".split(",");
		String[][] csvMatrix2=TimeSeriesSimulator.createReport(timeSeries,columns);
		TimeSeriesSimulator.writeCsv(new File("abcomm_A_B_IPTG_ATC.csv"), csvMatrix2);
